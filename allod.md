...
- `allod patch receive <vm>:<repo-id> <repo-id>` - the public/private relay: a human pulls an agent's committed patches over SSH and applies them where they can be pushed. Both ends take a registry id such as `allod/memory`, resolved through `inventory/scripts/repositories.json` on each machine, so the handed-over line carries no filesystem path; a path still works. There is no `--push`: the human pushes after looking. `fetch` and `apply` are its halves; see `allod/tools` `docs/allod-patch.md`
- `flake-status [input] [--upstream]` - inspect flake lock staleness before updates. `--upstream` compares each pin to the head of the branch its lock names (`original.ref`, or the default branch when none), by the same rules `flake-update-cascade` reads heads, so an arrow is a move that command can make
