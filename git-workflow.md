...
- Use `allod change` for change work instead of manual git and Forge mutation steps:
  ...
  - Open PRs with `allod change submit -t <title> -F <body-file>`; body must include `## Validation`.
...
- PR descriptions must include a `## Validation` section with exact commands the reviewer runs to verify the PR.
