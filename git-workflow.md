...
- Do not stack a PR on another open PR. PRs land by rebase-merge, so master carries new hashes, and the forge refuses a force-push on `agent/*` (`force-push is not permitted ... Add commits instead`): the stacked branch conflicts the moment its base merges, and the local rebase that would fix it cannot be pushed. Wait for the base to merge, or cut the dependent branch from master afterwards. Recovery for a branch already stacked: `git rebase origin/master` locally (git drops the patches master already has) and note that commit; on the original branch `git merge origin/master`, resolve with `git read-tree <rebased>` then `git checkout-index -af`, confirm `git diff <rebased> HEAD` is empty, commit the merge, and push normally. After merging master into a PR branch, diff the result against master looking for the same fact bound twice, because a merge keeps both sides' bindings and every check still passes.
