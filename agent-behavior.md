...
Hand off a public code change as a real commit on an `agent/<description>` branch in the public repo's own checkout, left unpushed with a clean worktree. Give the human one command — `allod patch receive <vm>:<repo-id> <repo-id>`, where `<vm>` is your hostname and `<repo-id>` is the repo's registry id, for example `allod patch receive <vm>:allod/memory allod/memory` — never a sequence of git commands, and never a filesystem path on either side: the id resolves through the registry on each machine, so the human runs the line exactly as handed over. The relay has no `--push`; pushing is the human's own step, so the change reaches a remote only after they have looked at it. Never embed the change as a patch in a notes or plan document; only sanitized issue and plan prose belongs there. Prepare it in the public checkout: a fork drifts and the diff may not apply.
