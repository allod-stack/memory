...
- After opening a PR, decide whether a separate read-only review pass is warranted by risk and blast radius. Require it for nontrivial code or generated-behavior changes, shared data or interface changes, security-sensitive changes, broad lock updates, or PR body claims that need independent verification. Skip it for trivial prose, memory, metadata, or narrow lock-only PRs after the commit range has already been inspected.
