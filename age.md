...
## A credential lands in three pieces or not at all

The registry entry, the recipient line in `secrets.nix`, and the `.age` file belong in one commit. The secrets repo's `credential-inventory` check asserts both directions, so any two without the third fails: a registered agenix consumer with no file reports `ERROR: missing <path> for <name>`, and a `secrets.nix` entry with no consumer record reports `credential-inventory: secrets missing consumer records: <path>`. agenix reads `secrets.nix` for recipients, so it refuses a path that file does not declare (`error: attribute '"<name>.age"' missing`) — which forces the order: write both entries, then encrypt, then commit all three.
