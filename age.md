...
An agent's PR against the secrets repo declares the non-secret half: the `credentials.nix` entry with `rotation_state = "pending"`, the `secrets.nix` recipient line, and the rotation registry group in `rotation-registry.json` (`service` is `forgejo` for a UI token, `none` for anything else). The `credential-inventory` check passes with the `.age` file absent while the entry is pending, and refuses a pending entry that has one or an active entry that lacks one, so the PR is green on its own and can never be merged half-landed. The operator then checks out that branch on the host and runs `allod secret create <name>` with the value on stdin: it encrypts to exactly the recipients `secrets.nix` declares, flips the entry to active, runs the checks, and commits and pushes the branch. `allod secret rekey <name>` re-encrypts after a recipient line changes; `allod secret rotate <name>` replaces the value across its rotation group and, for a group with `local_auth_refresh` entries, prints `refresh-local-auth --group <alias>` as the next step: that stays a nexus host script because it installs root-owned netrc files under sudo. The command exists only on the host build (`secret` tag); an agent's half is the PR and the one-line handover, never a plaintext. Details: `docs/allod-secret.md` in `allod/tools`.
