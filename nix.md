...
`netrc-file` in nix.conf covers Nix's own curl transfers — binary caches and tarball fetches — and not `git+https://` flake inputs.
...
The per-user netrc layout below is the libvirt path and is known to work, but its third entry is not the only way to authenticate a user's `nix flake update`:

- `/etc/nix/netrc` (640, root:users) - nix daemon, and any nix client's curl transfers, via `netrc-file` in nix.conf
- `/root/.netrc` (600, root:root) - root's own fetches
- `/home/${username}/.netrc` (600, user:users) - the user's own fetches

All three are generated by `archetypes/modules/netrc.nix` from `/root/.git-credentials`. A guest with no writable home — a microvm, where credential-derived plaintext lives under one runtime root — reaches the same result with a git credential helper instead.
...
Allocation is not peak. An expensive read that is consumed and dropped can add a lot of total work and none of the peak — which is what the thunk discipline above converts one into the other. Costing a read at "~0.5 GB" says what it allocates, not what removing it would save.
