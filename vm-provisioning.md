...
- **Host-key rotation does not re-key the installer ISO** - the installer config bakes the host public keys into the installer root, and provisioning SSHes the installer as root with the host key. Until the ISO is rebuilt, provisioning fails at `nixos-anywhere` with `Permission denied (publickey)`. Tracked as `allod/nexus` issues #8 and #9.
